//
//  FoodVendingMachine.swift
//  0712_FoodMission
//
//  Created by woowabrothers on 2017. 7. 12..
//  Copyright © 2017년 woowabrothers. All rights reserved.
//

import Foundation

enum Menu: Int {
    case chicken = 1, pizza, bossam, haumbuger, dduckppokki
}

class FoodVendingMachine3: NSObject, NSCoding {
    
    static var instance = FoodVendingMachine3()
    var menuDic: [Int:[Restaurant]]
    var buyDic: [Int:Int]
    var balance: Int
    
    private override init() {
        menuDic = [:]
        buyDic = [:]
        balance = 0
    }
    
    required init?(coder aDecoder: NSCoder) {
        menuDic = aDecoder.decodeObject(forKey: "menuList") as! [Int:[Restaurant]]
        buyDic = aDecoder.decodeObject(forKey: "buyList") as! [Int:Int]
        balance = aDecoder.decodeInteger(forKey: "balance")
    }
    
    func encode(with aCoder: NSCoder) {
        aCoder.encode(menuDic, forKey: "menuList")
        aCoder.encode(buyDic, forKey: "buyList")
        aCoder.encode(balance, forKey: "balance")
    }
    
    func add(money: Int) -> Int {
        balance += money
        print("현재 금액 : "+String(balance))
        
        return balance
    }
    
    func getAllCapcityList() -> [Int : Int] {
        var result: [Int : Int] = [:]
        
        for (menu, restaurantList) in menuDic {
            var allOfCapcity = 0
            for restaurant in restaurantList {
                allOfCapcity += restaurant.capacity
            }
            result[menu] = allOfCapcity
        }
        return result
    }
    
    func add(restaurant: String, food: Food) {
        let type: Menu? = self.getMenuType(food: food)
        
        if let menu = type {
            if var existMenu = menuDic[menu.rawValue] {
                var flag = false
                for shop in existMenu {
                    print("존재함", shop.name)
                    if shop.name == restaurant {
                        flag = true
                        shop.addFoodMenu(food: food); break
                    }
                }
                
                if flag == false {
                    print("존재하지 않음")
                    let instance = Restaurant(name: restaurant)
                    instance.addFoodMenu(food: food)
                    existMenu.append(instance)
                    
                }
            } else {
                print("메뉴도 새로")
                let instance = Restaurant(name: restaurant)
                instance.addFoodMenu(food: food)
                menuDic[menu.rawValue] = [instance]
            }
            
            NotificationCenter.default.post(name: Notification.Name("NotificationIdentifier2"), object: self, userInfo: ["menu":type!.rawValue, "capacitylist" : getAllCapcityList()])
        } else { print("error") }
    }
    
//    func getAvailablePurchaseFood(money: Int) -> [Food] {
//        var result = [Food]()
//        
//        for (_, foodlist) in menuDic {
//            for (_, food) in foodlist {
//                if(food.price <= money && food.capacity > 0) {
//                    result.append(food)
//                }
//            }
//        }
//        return result
//    }
    
    func buy(restaurant: String, food: Food) -> Int {
        let type:Menu? = getMenuType(food: food)
        if food.capacity > 0 {
            if food.price < balance {
                balance -= food.price
                food.capacity -= 1
                buyDic[(type?.rawValue)!] = food.price
                return balance
            }else {
                return -1
            }
        }else {
            print("error : 찾으시는 음식의 수량이 없습니다.")
            return -2
        }
        return -3
//        if let exist = menuDic[menu]?.first?.value {
//            if (exist.price) < balance {
//                balance -= exist.price
//                exist.capacity -= 1
//                buyDic[menu] = exist.price
//                return balance
//            } else {
//                print("금액이 부족합니다. 돈을 더 넣어주세요 :)")
//                return -1
//            }
//        } else {
//            print("error : 찾으시는 음식이 존재하지 않습니다.")
//            return -2
//        }
    }
    
    func getMenuType(food: Food) -> Menu? {
        var type: Menu? = nil
        if food.isKind(of: Chicken.self) {
            print("치킨타입")
            type = .chicken
        } else if food.isKind(of: Pizza.self) {
            print("피자타입")
            type = .pizza
        } else if food.isKind(of: Bossam.self) {
            print("보쌈타입")
            type = .bossam
        } else if food.isKind(of: Hambuger.self) {
            print("햄버거타입")
            type = .haumbuger
        } else if food.isKind(of: Dduckppokki.self) {
            print("떡볶이타입")
            type = .dduckppokki
        }
        return type
    }
    
    //그냥 String배열로 리턴
    func getBuyList() -> [Int:Int] {
        return buyDic
    }
}
